...
namespace TelemedSystem.Services
{
    ...
    public class BackupService : BackgroundService
    {
        ...
        public BackupService(ILogger<BackupService> logger, IConfiguration config)
        {
            ...
            _commandTimeoutSeconds = dbCfg.GetValue<int?>("CommandTimeoutSeconds") ?? 60 * 60;

            _logger.LogInformation(
                "BackupService initialized. Database: {db}, BackupDir: {dir}, ScheduleHours: {hrs}, RetentionDays: {days}",
...
WITH FORMAT, INIT, NAME = N'{_databaseName}-FullBackup-{timestamp}';";
        ...
        public Task RunBackupNowAsync()
        {
            return RunBackupCycle(CancellationToken.None);
        }
    }
}
